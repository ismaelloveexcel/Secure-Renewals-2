    ...
    <div className="h-screen bg-slate-50 flex items-center justify-center p-2 sm:p-4">
      <div className="w-full max-w-md h-full max-h-[700px] sm:max-h-[750px]">
        <div className="bg-white/90 backdrop-blur-md border border-slate-200/60 shadow-[0_8px_30px_rgba(0,0,0,0.06)] rounded-2xl sm:rounded-3xl overflow-hidden flex flex-col h-full">
          ...
          <div className="border-t border-slate-100 px-2 py-1.5 sm:py-2 flex-shrink-0 bg-white/90 backdrop-blur-sm">
            <div className="flex">
              {tabs.map(tab => (
                <button
                  key={tab.id}
                ...
                >
                  <div className={`p-1 sm:p-1.5 rounded-lg transition-all duration-200 ${activeTab === tab.id ? 'bg-emerald-50' : ''}`}>
                    <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={activeTab === tab.id ? 2 : 1.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d={tab.icon} />
                    </svg>
                  </div>
                  <span className={`text-[8px] sm:text-[9px] font-medium ${activeTab === tab.id ? 'font-bold' : ''}`}>{tab.label}</span>
                  {activeTab === tab.id && (
