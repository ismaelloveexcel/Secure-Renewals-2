      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white/80 backdrop-blur-md border border-slate-200/60 shadow-[0_8px_30px_rgba(0,0,0,0.06)] rounded-3xl overflow-hidden">
          ...
          <div className="mx-4 mb-4">
            <p className="text-[10px] uppercase tracking-widest text-slate-400 mb-3">Journey</p>
            <div className="flex items-center gap-1">
              {['applied', 'screening', 'assessment', 'interview', 'offer'].map((stage, idx) => {
                const isCompleted = idx < currentStageIndex
                const isCurrent = idx === currentStageIndex
                return (
                  <div key={stage} className="flex-1 flex flex-col items-center">
                    <div className={`h-1.5 w-full rounded-full mb-1.5 ${
                      isCompleted ? 'bg-emerald-500' : isCurrent ? 'bg-emerald-500' : 'bg-slate-200'
                    }`}></div>
                    <span className={`text-[9px] ${
                      isCompleted || isCurrent ? 'text-slate-600' : 'text-slate-400'
                    }`}>{stageLabels[stage]?.substring(0, 8)}...</span>
                  </div>
                )
              })}
            </div>
